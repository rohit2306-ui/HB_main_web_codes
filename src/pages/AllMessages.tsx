...
const CommunitiesPage: React.FC = () => {
      ...
      try {
        ...
        if (user) {
          const joinedSet = new Set(
            list.filter((c) => c.members.includes(user.id)).map((c) => c.id)
          );
  ...
  const handleJoinCommunity = async (communityId: string) => {
    ...
    try {
      ...
      setCommunities((prev) =>
        prev.map((c) =>
          c.id === communityId
            ? { ...c, members: c.members ? [...c.members, user.id] : [user.id] }
            : c
        )
      );
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-gray-50  dark:bg-gray-900 text-gray-900 dark:text-gray-100">
      {/* Header */}
      <div className="bg-white dark:bg-gray-800 pt-20 border-b border-gray-200 dark:border-gray-700 sticky top-0 z-40 backdrop-blur-lg bg-white/90 dark:bg-gray-800/90">
